    ...
    <%
        ...
        if (courseId != null) {
            try {
                ...
                // Query to get videos related to the selected course
                String videoQuery = "SELECT video_title, video_url FROM videos WHERE course_id = ?";
                PreparedStatement videoStmt = con.prepareStatement(videoQuery);
                videoStmt.setInt(1, Integer.parseInt(courseId));
                ResultSet videoRs = videoStmt.executeQuery();
    %>
                <h3>Videos for <%= courseName %>:</h3>
                <% 
                    // Check if there are videos for the course
                    if (!videoRs.isBeforeFirst()) {
                        out.println("<p><center>No videos available for this course.</center></p>");
                    } else {
                %>
                <ul class="video-list">
                <%
                    // Loop through all the videos and display them using iframe
                    while (videoRs.next()) {
                        String videoTitle = videoRs.getString("video_title");
                        String videoLink = videoRs.getString("video_url");
                        
                        // Extract YouTube video ID from the URL if it is YouTube
                        String videoId = "";
                        if (videoLink.contains("youtube.com")) {
                            String[] urlParts = videoLink.split("v=");
                ...
                %>
                <div class="complete-course-form">
